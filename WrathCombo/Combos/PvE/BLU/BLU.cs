...
internal partial class BLU
{
    ...
    internal class BLU_PerpetualRayStunCombo : CustomCombo
    {
        ...
        protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level) => (actionID is PerpetualRay && (TargetHasEffectAny(Debuffs.Stun) || WasLastAction(PerpetualRay)) && IsSpellActive(SharpenedKnife) && InMeleeRange()) ? SharpenedKnife : actionID;
    }
    ...
    internal class BLU_MeleeCombo : CustomCombo
    {
        ...
        protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level) => (actionID is SonicBoom && GetTargetDistance() <= 3 && IsSpellActive(SharpenedKnife)) ? SharpenedKnife : actionID;
    }
