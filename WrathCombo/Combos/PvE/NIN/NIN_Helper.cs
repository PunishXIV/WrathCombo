...
internal partial class NIN
{
    ...
    internal static MudraFlags Flags => LocalPlayer.HasStatus(Buffs.Mudra, out var _, false) ? (MudraFlags)LocalPlayer.Status(Buffs.Mudra).Param : LocalPlayer.HasStatus(Buffs.TenChiJin, out var _, false) ? (MudraFlags)LocalPlayer.Status(Buffs.TenChiJin).Param : MudraFlags.None;
    ...
    internal static bool HasDoton => LocalPlayer.HasStatus(Buffs.Doton, out var _, false);
    ...
    internal static bool CanUseSuiton => ActionLearned(Suiton) && ActionReady(Ten) && !LocalPlayer.HasStatus(Buffs.ShadowWalker, out var _, false);

    internal static bool CanUseHuton => ActionLearned(Huton) && ActionReady(Ten) && !LocalPlayer.HasStatus(Buffs.ShadowWalker, out var _, false);
    ...
    internal static bool CanPhantomKamaitachi => !MudraPhase && LocalPlayer.HasStatus(Buffs.PhantomReady, out var _, false) &&
    ...
    internal static bool CanThrowingDaggersAoE => !MudraPhase && ActionReady(ThrowingDaggers) && HasTarget() && GetTargetDistance() >= 4.5 && InActionRange(ThrowingDaggers) &&
                                                  !LocalPlayer.HasStatus(Buffs.RaijuReady, out var _, false);
    internal static bool CanRaiju => !MudraPhase && LocalPlayer.HasStatus(Buffs.RaijuReady, out var _, false);
    ...
    internal static bool CanTrickST => ActionReady(OriginalHook(TrickAttack)) && CanWeave() && CurrentTarget.CanApplyStatus([Debuffs.TrickAttack, Debuffs.KunaisBane]) && LocalPlayer.HasStatus(Buffs.ShadowWalker, out var _, false) && !MudraPhase &&
                                       (MugDebuff || MugCD >= 45 || MugDisabledST);
    internal static bool CanTrickAoE => ActionReady(OriginalHook(TrickAttack)) && CanWeave() && CurrentTarget.CanApplyStatus([Debuffs.TrickAttack, Debuffs.KunaisBane]) && LocalPlayer.HasStatus(Buffs.ShadowWalker, out var _, false) && !MudraPhase &&
    ...
    internal static bool TrickDebuff => CurrentTarget.HasStatus(Debuffs.TrickAttack, out var _, false) || CurrentTarget.HasStatus(Debuffs.KunaisBane, out var _, false) || JustUsed(OriginalHook(TrickAttack));
    internal static bool MugDebuff => CurrentTarget.HasStatus(Debuffs.Mug, out var _, false) || CurrentTarget.HasStatus(Debuffs.Dokumori, out var _, false) || JustUsed(OriginalHook(Mug));
    ...
    internal static bool CanBhavacakra => CanWeave() && gauge.Ninki >= 50 && !MudraPhase &&
                                          (!LocalPlayer.HasStatus(Buffs.Higi, out var _, false) || BuffWindow || TrickDisabledST);
    internal static bool CanHellfrogMedium => CanWeave() && gauge.Ninki >= 50 && ActionLearned(HellfrogMedium) && !MudraPhase &&
                                              (!LocalPlayer.HasStatus(Buffs.Higi, out var _, false) || BuffWindow || TrickDisabledAoE);
    ...
    internal static int NinkiPool()
    {
        ...
        if (TrickDebuff)
            return 50;
        if (LocalPlayer.HasStatus(Buffs.Bunshin, out var _, false))
    ...
    internal static bool HasKassatsu => LocalPlayer.HasStatus(Buffs.Kassatsu, out var _, false) || JustUsed(Kassatsu, 1);
    internal static bool HasShadowWalker => LocalPlayer.HasStatus(Buffs.ShadowWalker, out var _, false) || JustUsed(Suiton, 1) || JustUsed(Huton, 1) || JustUsed(TCJSuiton, 1) || JustUsed(TCJHuton, 1);
    ...
    internal static bool CanTenriJindo => CanWeave() && LocalPlayer.HasStatus(Buffs.TenriJendoReady, out var _, false);
    ...
    internal class MudraCasting
    {
        ...
        public bool CastFumaShuriken(ref uint actionID) // Ten
        {
            if (CurrentMudra is MudraState.None or MudraState.CastingFumaShuriken)
            {
                ...
                actionID = LocalPlayer.HasStatus(Buffs.Kassatsu, out var _, false) ? TenCombo : Ten;
                return true;
            }
            CurrentMudra = MudraState.None;
            return false;
        ...
        public bool CastKaton(ref uint actionID) // Jin Ten
        {
            if (Katon.LevelChecked() && CurrentMudra is MudraState.None or MudraState.CastingKaton)
            {
                ...
                actionID = LocalPlayer.HasStatus(Buffs.Kassatsu, out var _, false) ? ChiCombo : Chi;
                return true;
            }
            CurrentMudra = MudraState.None;
            return false;
        ...
        public bool CastDoton(ref uint actionID) // Jin Ten Chi
        {
            if (Doton.LevelChecked() && CurrentMudra is MudraState.None or MudraState.CastingDoton)
            {
                ...
                actionID = LocalPlayer.HasStatus(Buffs.Kassatsu, out var _, false) ? JinCombo : Jin;
                return true;
            }
            CurrentMudra = MudraState.None;
            return false;
